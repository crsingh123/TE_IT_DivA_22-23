...
def main():
    ...
    st.subheader("Enter the image containing the fake news by clicking on the button below")
    ...
    if is_user_logged_in():
        upload_image = st.file_uploader("Select the image containing news and click on upload")
        if upload_image is not None:
            image = Image.open(upload_image)
            st.image(image, caption='Uploaded Image', use_column_width=True)
            if st.button('Convert Image'):
                news = pytesseract.image_to_string(image)
                st.write(news)
